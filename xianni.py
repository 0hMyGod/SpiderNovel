...
class Book_spider():
    url = 'http://www.quanshuwang.com/book/15/15228'
    headers = [
    ...
    '''
    def items_list(self):
        book_request = self.session.get(self.url, headers=random.choice(self.headers))
        ...
        for item in book_divs:
            if 'book/15' in item.attrs['href']:  # 筛选掉无用的a标签连接
                time.sleep(1)  # 暂停三秒防止ip被封
                title = item.text
    ...
    def item_content(self, item_url, title):
        content_request = self.session.get(item_url, headers=random.choice(self.headers))
        if content_request.status_code == 200:
            ...
            if content:       
                # 将小说保存到txt文件中
                with open('xzhdx.txt', 'a', encoding='utf-8') as f:  
                    f.write('\n'+title+'\n')
                    ...
                    for info in info_text:                
                        if info != '\n' and info != '':
                            ...
                            info.replace(u'\xa0', u' ')  
                            用此方法可保存，后面的两个替换是因为每页都有这样的两个多余字符串
                            '''                        
